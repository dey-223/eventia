...
interface PageProps {
  ...
  [key: string]: any; // Pour satisfaire la contrainte Inertia
}
...
const EventParticipants: React.FC = () => {
  ...
  const handleDeleteParticipant = () => {
    if (participantToDelete === null) return;
    // TODO: appeler votre API pour supprimer en base
    toast.success('Participant supprimé avec succès');
    setDeleteDialogOpen(false);
    setParticipantToDelete(null);
  };
  ...
  const handleToggleAttendance = (participantId: number) => {
    // TODO: appeler votre API pour basculer le statut en base
    const p = participants.find(p => p.id === participantId);
    if (p) {
      toast.success(`Statut de présence mis à jour pour ${p.name}`);
    }
  };
                ...
                {filteredParticipants.length > 0 ? (
                  filteredParticipants.map(p => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell className="text-right space-x-2">
                        <Button size="icon" variant="ghost" onClick={() => handleToggleAttendance(p.id)}>
                          {p.attended ? <X className="w-4 h-4 text-red-500" /> : <Check className="w-4 h-4 text-green-500" />}
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => openDeleteDialog(p.id)}>
                          <Trash2 className="w-4 h-4 text-red-600" />
                        </Button>
                      </TableCell>
                    </TableRow>
