...
const EventSignup: React.FC = () => {
  const { props } = usePage<PageProps>();
  const { event, errors: pageErrors, flash } = props;
  const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  useEffect(() => {
    if (pageErrors) {
      Object.entries(pageErrors).forEach(([key, value]) => {
        if (key === 'event_full' || key === 'already_registered') {
          toast.error(value);
          form.setError('acceptTerms', { type: 'server', message: value });
        } else {
          form.setError(key as keyof FormValues, { type: 'server', message: value });
        }
      });
  ...
  const onSubmit = (values: FormValues) => {
    setIsSubmitting(true);
    router.post(
      route('events.register', { id: event.id_event }),
      values,
      {
        onError: () => {
          toast.error("Échec de l'inscription. Vérifiez le formulaire.");
        },
        onFinish: () => {
          setIsSubmitting(false);
        },
  ...
  // Calcul du nombre actuel de participants et état complet
  const current = event.registrations_count ?? event.participantsCount ?? 0;
  const isEventFull = current >= event.maxParticipants;
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Nom complet</FormLabel>
                        <FormControl>
                          <Input {...field} disabled={isSubmitting || isEventFull} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Email</FormLabel>
                        <FormControl>
                          <Input type="email" {...field} disabled={isSubmitting || isEventFull} />
                        </FormControl>
                        <FormDescription>Votre billet sera envoyé à cette adresse</FormDescription>
                        <FormMessage />
                      </FormItem>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <Users className="h-4 w-4" />
                  <span>
                    {current}/{event.maxParticipants} participants{isEventFull && ' (Complet)'}
                  </span>
                </div>
              </div>
              <Separator />
